...
export function classifyDictionaryWords<T extends DictionaryWord>(entries: T[]): ClassifiedDictionaryWord<T>[] {
  ...
  const highSchool = [...unique.values()].filter((entry) => entry.tags.includes("gk")).sort(byFrequency);
  ...
  const classifiedHighSchool = highSchool.map((entry, index) => ({
    ...entry,
    wordBook: (index < firstBoundary ? "grade1" : index < secondBoundary ? "grade2" : "grade3") as WordBookId,
    position: index < firstBoundary ? index + 1 : index < secondBoundary ? index - firstBoundary + 1 : index - secondBoundary + 1,
  }));
  const classifiedCet4 = cet4Only.map((entry, index) => ({ ...entry, wordBook: "cet4" as const, position: index + 1 }));
