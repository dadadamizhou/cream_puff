...
import { buildWordLibraryEndpoint, wordLibraryViewReducer } from "@/lib/word-library-view";
...
export function WordLibrary({ initialStatus, initialScope, initialDate, initialBook }: { initialStatus: WordLibraryStatus; initialScope: WordLibraryScope; initialDate: string | null; initialBook: WordLibraryBook }) {
  const [view, dispatch] = useReducer(wordLibraryViewReducer, {
  ...
  const { status, book, scope, date } = view;
  const form = useForm<SearchInput>({ resolver: zodResolver(searchSchema), defaultValues: { q: "" } });
  const endpoint = buildWordLibraryEndpoint(view);
  const { data, error, isLoading, isValidating, mutate } = useSWR(endpoint, (url: string) => api.get<WordLibraryData>(url).then((response) => response.data));
  ...
  function search(values: SearchInput) {
    dispatch({ type: "search", query: values.q });
  }

  const dayLabel = date ? formatDateLabel(date) : null;
  const currentPage = data?.pagination.page ?? view.page;
  ...
  return <div className="pb-4"><header className="mb-5 flex items-center gap-3"><Link href="/study" aria-label="返回学习首页" className="grid size-11 shrink-0 place-items-center rounded-2xl border border-line bg-card text-muted"><ArrowLeft className="size-5" /></Link><div><p className="text-sm text-muted">{scope === "day" && dayLabel ? `${dayLabel}的真实学习记录` : "每个词都有自己的进度"}</p><h1 className="mt-0.5 text-2xl font-bold tracking-tight">我的词库</h1></div></header>
    ...
    {data && data.pagination.totalPages > 1 && <div className="mt-5 flex items-center justify-between rounded-2xl border border-line bg-card p-2"><button type="button" disabled={isValidating || currentPage <= 1} onClick={() => dispatch({ type: "page", page: currentPage - 1 })} className="flex h-10 items-center gap-1 rounded-xl px-3 text-xs font-bold text-muted disabled:opacity-30"><ChevronLeft className="size-4" />上一页</button><span className="text-xs text-muted">{currentPage} / {data.pagination.totalPages}</span><button type="button" disabled={isValidating || currentPage >= data.pagination.totalPages} onClick={() => dispatch({ type: "page", page: currentPage + 1 })} className="flex h-10 items-center gap-1 rounded-xl px-3 text-xs font-bold text-brand disabled:opacity-30">下一页<ChevronRight className="size-4" /></button></div>}
  </div>;
}
...
function WordRow({ word }: { word: WordLibraryData["items"][number] }) {
  ...
  return <article className="flex min-h-20 items-center gap-3 rounded-2xl border border-line bg-card p-4"><div className="min-w-0 flex-1"><div className="flex flex-wrap items-baseline gap-x-2"><h2 className="break-all text-lg font-bold">{word.spelling}</h2>{word.phonetic && <span className="text-xs text-muted">{word.phonetic}</span>}{word.wordBooks.map((wordBook) => <span key={wordBook} className={cn("rounded px-1.5 py-0.5 text-[10px] font-semibold", wordBook === word.wordBook ? "bg-brand-soft text-brand" : "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300")}>{getWordBook(wordBook).shortLabel}</span>)}</div><p className="mt-1 line-clamp-2 text-xs leading-5 text-muted">{word.definition}</p></div><span className={cn("flex shrink-0 items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-bold", meta.className)}><Icon className="size-3" />{meta.label}</span></article>;
}
