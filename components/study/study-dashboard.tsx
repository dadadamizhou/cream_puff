...
import { BookOpen, CalendarCheck2, Cat, Check, ChevronRight, Flame, LoaderCircle, Mic2, PawPrint, Play, RotateCcw, Sparkles, Volume2, X } from "lucide-react";
...
export function StudyDashboard() {
  ...
  const { data: practiceData } = useSWR("/api/practice/today", practiceFetcher, { revalidateOnFocus: false, shouldRetryOnError: false });
...
function StudySession({ tasks, onClose, onComplete }: { tasks: StudyTask[]; onClose: () => void; onComplete: () => void }) {
  ...
  return <div className="mx-auto max-w-xl"><header className="mb-5 flex items-center gap-4"><button onClick={onClose} aria-label="退出学习" className="grid size-10 place-items-center rounded-xl bg-card text-muted"><X className="size-5" /></button><div className="h-2 flex-1 overflow-hidden rounded-full bg-line"><div className="h-full rounded-full bg-brand transition-all" style={{ width: `${pct}%` }} /></div><span className="w-12 text-right text-xs font-semibold text-muted">{currentIndex + 1}/{tasks.length}</span></header>
    ...
    <p className="mt-4 text-center text-xs text-muted">{revealed ? "根据刚才的回忆情况选择" : "先大声跟读并回忆，再查看释义"}</p>
    <div className="mt-2 grid grid-cols-4 gap-2">{([{ id: "again", label: "忘记", next: "10 分钟" }, { id: "hard", label: "模糊", next: "明天" }, { id: "good", label: "记得", next: "下一阶" }, { id: "easy", label: "熟练", next: "跨两阶" }] as const).map((item) => <button key={item.id} disabled={submitting || !revealed} onClick={() => rate(item.id)} className={cn("h-16 rounded-xl border border-line bg-card text-xs font-bold transition disabled:cursor-not-allowed disabled:opacity-35", item.id === "good" && "border-brand bg-brand text-white")}><span className="block">{item.label}</span><span className={cn("mt-1 block text-[10px] font-normal text-muted", item.id === "good" && "text-white/75")}>{item.next}</span></button>)}</div>
  </div>;
}
