...
function PracticeOverview({ data, onStart }: { data: PracticeTodayData; onStart: () => void }) {
  const typeCounts = data.questions.reduce<Partial<Record<PracticeType, number>>>((counts, question) => {
    counts[question.type] = (counts[question.type] ?? 0) + 1;
    return counts;
  }, {});

  return <div className="pb-4"><div className="mb-6"><p className="text-sm text-muted">把记住变成会用</p><h1 className="mt-1 text-2xl font-bold tracking-tight">每日一练</h1></div><section className="relative overflow-hidden rounded-2xl bg-brand p-5 text-white shadow-lg shadow-pink-200"><PawDecoration light /><div className="relative"><div className="flex items-center justify-between"><span className="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold">今日挑战</span><Cat className="size-6" /></div><div className="mt-8 flex items-end gap-2"><strong className="text-4xl">{data.summary.remaining}</strong><span className="mb-1 text-sm text-white/80">题待完成</span></div><p className="mt-2 text-xs text-white/75">已答 {data.summary.answered}/{data.summary.total} · 当前正确率 {data.summary.accuracy}%</p><p className="mt-1 text-xs text-white/65">按已学词量动态安排，今天约 {Math.max(5, Math.ceil(data.summary.total * 0.6))} 分钟</p><button onClick={onStart} className="mt-6 flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-white font-bold text-brand"><PawPrint className="size-4" />{data.summary.answered ? "继续练习" : "开始练习"}</button></div></section><section className="mt-5"><div className="mb-3 flex items-center justify-between"><h2 className="font-bold">今天练什么</h2><span className="text-xs text-muted">共 {data.summary.total} 题</span></div><div className="grid grid-cols-2 gap-3">{(Object.entries(TYPE_META) as [PracticeType, (typeof TYPE_META)[PracticeType]][]).map(([type, meta], index) => { const Icon = meta.icon; return <div key={type} className={cn("flex min-h-20 items-center gap-3 rounded-2xl border border-line bg-card p-3", index === 4 && "col-span-2")}><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-brand-soft text-brand"><Icon className="size-5" /></span><div><p className="text-sm font-bold">{meta.short}</p><p className="mt-1 text-[11px] text-muted">{meta.label} · {typeCounts[type] ?? 0} 题</p></div></div>; })}</div></section></div>;
}
