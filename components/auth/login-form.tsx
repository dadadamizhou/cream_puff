...
export function LoginForm() {
  ...
  return (
    <section className="mx-auto w-full max-w-md">
      <div className="mb-8 flex items-center gap-3"><div className="grid size-11 place-items-center rounded-2xl bg-brand text-white shadow-lg shadow-pink-200"><Sparkles className="size-5" /></div><div><p className="text-lg font-bold tracking-tight">泡芙</p><p className="text-xs text-muted">每天跟读一点，记住该记的</p></div></div>
      <div className="rounded-3xl border border-line bg-card p-6 shadow-sm sm:p-8">
        <div className="mb-7"><h1 className="text-2xl font-bold tracking-tight">{mode === "login" ? "继续你的学习" : "建立你的词库"}</h1><p className="mt-2 text-sm leading-6 text-muted">每天新学一点、按时复习一点，把高中核心词汇真正变成长时记忆。</p></div>
        ...
        <form className="space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <button disabled={loading} className="mt-2 flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-brand text-sm font-bold text-white shadow-lg shadow-pink-200 transition hover:bg-pink-700 disabled:cursor-not-allowed disabled:opacity-60">{loading ? <LoaderCircle className="size-4 animate-spin" /> : <>{mode === "login" ? "进入学习" : "创建账号"}<ArrowRight className="size-4" /></>}</button>
        </form>
        <p className="mt-6 text-center text-xs leading-5 text-muted">继续即表示你同意泡芙仅用于学习进度记录，不会出售你的个人数据。</p>
      </div>
